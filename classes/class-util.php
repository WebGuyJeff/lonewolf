...
class Util {
	...
	public static function enqueue_assets( ...$fonts ) {
		...
		if ( ! isset( $fonts ) ) {
			echo '<script>console.log("ERROR: BigupWeb\Lonewolf\Util:enqueue_assets() FONT ARGS EMPTY")</script>';
			...
			foreach ( $fonts as $font_name ) {
				switch ( $font_name ) {
					...
					default:
						echo '<script>console.log("ERROR: Jefferson\\\Lonewolf\\\Util:enqueue_assets() FONT NOT FOUND: ' . $font_name . '")</script>';
						break;
				}
			}
