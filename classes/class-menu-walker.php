<?php
/**
 * Lonewolf Custom Menu_Walker Class.
 *
 * @package lonewolf
...
class Menu_Walker extends Walker_Nav_Menu {
	...
	public function display_element( $item, &$children_elements, $max_depth, $depth, $args, &$output ) {
		...
		if ( array_key_exists( $id, $children_elements ) ) {
			// ...it's a parent.
			$item->lw__is_parent = true;
			// Keep (unreliable) WP var correct because we're nice.
			$this->has_children = $item->lw__is_parent;
		...
		if ( $item->current && ! $this->is_search ) {
			...
			$item->lw__is_active = true;
		} else {
			// Explicit boolean.
			$item->lw__is_active = false;
		}
		...
		if ( ( 0 === $max_depth || $max_depth > $depth + 1 ) && $item->lw__is_parent ) {
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		$is_parent = $item->lw__is_parent;
		$is_active = $item->lw__is_active;
...
TEMPLATE;
	...
	public function build_class_string( $item, $depth, $args ) {
		...
		$is_parent  = $item->lw__is_parent;
		$is_active  = $item->lw__is_active;
		$has_active = $item->lw__has_active;
